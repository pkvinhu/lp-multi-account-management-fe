...
): ThunkAction<void, RootState, null, ProfileAction | any> => {
  return async dispatch => {
    try {
      const res: any = await axios.get(
        `http://localhost:1337/api/profiles/${account}`,
        {
      ...
      res.data.forEach((e, i) => {
        roleTypeCountMap[e.roleTypeName]
          ? roleTypeCountMap[e.roleTypeName]++
          : (roleTypeCountMap[e.roleTypeName] = 1);
        map[e.id] = e.name;
      });
      dispatch({
        type: GET_PROFILES,
